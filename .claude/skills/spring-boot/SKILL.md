---
name: spring-boot
description: Spring Boot 4.x / Java 25 development - REST APIs, DynamoDB, Security, Testing, and Cloud-native patterns. Use for building enterprise Java applications with Spring Boot.
metadata:
  version: "3.0.0"
  domain: backend
  triggers: Spring Boot, Spring Framework, Spring Security, Spring Data, Spring WebFlux, Java REST API, Microservices Java
  role: specialist
...
Enterprise Spring Boot 4.x / Java 25 development with focus on clean architecture and production-ready code.

## Project Stack (DaftieScanner)
- Spring Boot 4.0.5, Java 25, Maven
- AWS SDK v2 with `dynamodb-enhanced` (no JPA — DynamoDB only)
- Testcontainers with `amazon/dynamodb-local:2.5.2` for ITs
- Lombok: `@Slf4j`, `@Data`, `@RequiredArgsConstructor`, `@UtilityClass`
- YAML config only (no `.properties` files)
- `RestClient` (not `WebClient`) with `JdkClientHttpRequestFactory`
...
Spring Boot 4.x, Java 25, Spring WebFlux, Project Reactor, Spring Data JPA, Spring Security 6, OAuth2/JWT, Hibernate, R2DBC, AWS DynamoDB Enhanced, Spring Cloud, Resilience4j, Micrometer, JUnit 5, TestContainers, Mockito, Maven
