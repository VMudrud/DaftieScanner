package com.vmudrud.daftiescanner.search.metrics;
...
public class MetricsPublisher {
    ...
    public void recordBlockDetected(String tenantId) {
        counter(METRIC_BLOCK_DETECTED, TAG_TENANT_ID, tenantId).increment();
    }

    public void recordPollError(String tenantId, String errorType) {
        counter(METRIC_POLL_ERRORS, TAG_TENANT_ID, tenantId, TAG_ERROR_TYPE, errorType).increment();
    }

    public void recordListingsFound(String tenantId, int count) {
        counter(METRIC_LISTINGS_FOUND, TAG_TENANT_ID, tenantId).increment(count);
    }

    public void recordPollDuration(String tenantId, long elapsedMs) {
